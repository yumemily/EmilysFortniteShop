        ...
        return (
            <Card className={`card ${item.rarity.name || "Standard"}`} >
                <Card.Title className='p-3'>
                    <img style={{ width: '2rem', paddingRight: '.5rem' }} src='https://fortnite-public-files.theapinetwork.com/fortnite-vbucks-icon.png'alt="v-bucks-icon" />
                    {item.price}
                </Card.Title>
                <Card.Img className='p-2' variant="top" src={item.images.icon} />
                <Card.Footer className='cardfooter'>
                    <Link className='stretched-link' to={`/shop/${item.id}`}>{item.name}
                </Link>
                </Card.Footer>
            </Card>
        )
        ...
        <div className='mb-5'>
           ...
           <Container>
                ...
                <div className="jumbotron-container">
                    LOREM IPSUM DOLOR SIT AMET, CONSECTETUR ADIPISCING ELIT. SI EST NIHIL NISI ELIT
                </div>
            </div>
